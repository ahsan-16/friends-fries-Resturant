...
export function ItemModal({ item, onClose }) {
  ...
  const [qty, setQty] = useState(1);
  const [selectedVariant, setSelectedVariant] = useState(
    item.variants && item.variants.length > 0 ? item.variants[0].label : null
  );

  const currentPrice = item.variants && item.variants.length > 0
    ? Number(item.variants.find((v) => v.label === selectedVariant)?.price ?? item.price)
    : Number(item.price);
      ...
      >
        ...
        <div className="relative bg-brand-muted p-6 pb-4">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="text-right shrink-0">
              <div className="price-tag text-2xl font-display">
        ...
        <div className="p-6 space-y-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {item.variants.map((v) => (
                  ...
                  >
                    {v.label}
                    <span className="block text-xs font-body opacity-80">
          ...
          {item.tags && item.tags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {item.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-xs px-2 py-1 bg-brand-muted text-gray-400 rounded-full font-body capitalize"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3 bg-brand-muted rounded-full px-3 py-2">
              ...
              <span className="font-heading font-bold text-white w-5 text-center">
                {qty}
              </span>
              <button
                onClick={() => setQty(qty + 1)}
            ...
            >
              <ShoppingCart size={18} />
              Add — PKR {(currentPrice * qty).toLocaleString()}
            </button>
          </div>
        </div>
      </div>
  ...
  const quickAdd = (e) => {
    ...
    dispatch({
      ...
      item: {
        ...
        emoji: item.emoji || "🍔",
        category: item.category,
        selectedVariant: item.variants && item.variants.length > 0
    ...
    <>
      ...
      >
        <div className="p-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="price-tag font-heading font-bold text-lg">
