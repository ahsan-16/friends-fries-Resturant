    ...
    <main className="min-h-screen pt-20">
      ...
      <div className="relative py-16 px-4 bg-gradient-to-b from-brand-red/10 to-transparent">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-brand-red font-heading font-bold uppercase tracking-widest text-sm mb-2">Best Value</p>
          <h1 className="font-display text-6xl md:text-8xl text-white">HOT DEALS</h1>
          <p className="text-gray-500 font-body mt-2">{deals.length} amazing combos at unbeatable prices</p>
        </div>
      </div>
          ...
          {deals.map((deal, i) => (
            ...
            >
              ...
              <div className="bg-gradient-to-r from-brand-red to-red-700 p-4 flex items-center justify-between">
                <div>
                  <span className="text-white/60 font-heading text-xs uppercase tracking-widest">Deal</span>
                  <div className="font-display text-4xl text-white leading-none">{String(i + 1).padStart(2, "0")}</div>
                </div>
                <div className="text-right">
                  <div className="text-white/60 text-xs font-body">From only</div>
                  <div className="font-display text-3xl text-brand-yellow">PKR {deal.price.toLocaleString()}</div>
                </div>
              </div>
              ...
              <div className="p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-heading font-bold text-white text-lg">{deal.name}</h3>
                </div>

                {/* Items list */}
                <ul className="space-y-1.5 mb-5">
                  {deal.items.map((item, j) => (
                    <li key={j} className="flex items-center gap-2 text-sm text-gray-300 font-body">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-brand-muted">
                  <div>
                    <div className="price-tag font-display text-2xl">PKR {deal.price.toLocaleString()}</div>
                    {deal.badge && <span className="badge text-xs mt-1 inline-block">{deal.badge}</span>}
                  </div>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      dispatch({
                        type: "ADD_ITEM",
                        item: { id: deal.id, name: deal.name, price: deal.price, emoji: "🔥", category: "deals", selectedVariant: null, qty: 1 },
                      });
