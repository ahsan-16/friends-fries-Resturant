...
export default function MenuPage({ title, subtitle, emoji, items, tags }) {
    ...
    <main className="min-h-screen pt-20">
      ...
      <div className="relative py-16 px-4 bg-gradient-to-b from-brand-gray to-transparent">
        ...
        <div className="max-w-7xl mx-auto text-center relative z-10">
          ...
          <p className="text-brand-red font-heading font-bold uppercase tracking-widest text-sm mb-2">{subtitle}</p>
          <h1 className="font-display text-6xl md:text-8xl text-white">{title}</h1>
          <p className="text-gray-500 font-body mt-2">{filtered.length} items available</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pb-20">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-heading font-bold uppercase tracking-wide transition-all ${
                  activeTag === "all" ? "bg-brand-red text-white" : "bg-brand-gray border border-brand-muted text-gray-400 hover:text-white"
                }`}
              >
                All
              </button>
              {tags.map((tag) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-heading font-bold uppercase tracking-wide transition-all capitalize ${
                    activeTag === tag ? "bg-brand-red text-white" : "bg-brand-gray border border-brand-muted text-gray-400 hover:text-white"
                  }`}
                >
                  {tag}
                </button>
        ...
        {filtered.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 text-sm font-body mt-1">Try a different search</p>
          </div>
        )}
      </div>
    </main>
